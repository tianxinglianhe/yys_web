...
<div class="container-fluid">
	...
	<div class="row">
		<div class="col-md-4">
			<h3>用户信息</h3>
			<p>收货人（公司名称）：<?php echo ($ofInfo['username']); ?></p>
			<p>
				收货地址：
				<?php echo ($ofInfo['take_over_province']); ?>
				<?php echo ($ofInfo['take_over_city']); ?>
				<?php echo ($ofInfo['take_over_town']); ?>
				<?php echo ($ofInfo['take_over_ex']); ?>
			</p>
			<p>联系电话：<?php echo ($ofInfo['tel_no']); ?></p>
		</div>
		<div class="col-md-4">
			<h3>订单信息</h3>
			<p>订单编号：<?php echo ($ofInfo['no']); ?></p>
			<p>下单时间：<?php echo a4getDatetime($ofInfo['create_time'],'datetime');;?></p>
			<p>
				总金额：
				<span class="glyphicon glyphicon-usd" style="color: #D20000;"> <strong>
					<?php echo ($ofInfo['price']); ?>
				</strong></span>
			</p>
		</div>
		<div class="col-md-4">
			<h3>打印订单</h3>
			<a href="<?php echo U('home/pay/printOutOrderForm');?>?id=<?php echo $_GET['id'];?>" class="btn btn-success btn-lg">打印订单</a>
		</div>
	</div>
	<div class="row" style="margin-top: 55px;">
		<table class="table">
			<tr class="warning">
				<td colspan="6"><h4>订单商品</h4></td>
			</tr>
			<tr class="info">
				<td>商品</td>
				<td>类别</td>
			...
			<?php foreach($ogInfo as $item): ?>
			<tr>
				<td>
					<img src="<?php echo W_PUBLIC_GOODS_IMG; echo ($item['goods_id']); ?>/<?php echo ($item['img']); ?>" alt="<?php echo ($item['name']); ?>" style="width: 200px;">
					<p><?php echo ($item['name']); ?></p>
				</td>
				<td><?php echo ($item['type']); ?></td>
				<td><?php echo ($item['norms']); ?></td>
				<td><?php echo ($item['number']); ?></td>
				<td><?php echo ($item['once_price']); ?></td>
				<td><?php echo ($item['number'] * $item['once_price']); ?></td>
			</tr>
			<?php endforeach; ?>
		</table>
	</div>
