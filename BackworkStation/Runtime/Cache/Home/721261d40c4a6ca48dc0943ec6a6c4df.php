	...
	<div class="row">
		<ul class="nav nav-tabs nav-justified">
			<li role="presentation">
				<a href="<?php echo U('home/pay/index');?>">所有订单</a>
			</li>
			<li role="presentation">
				<a href="<?php echo U('home/pay/isPay');?>">已付款</a>
			</li>
			<li role="presentation">
				<a href="<?php echo U('home/pay/unPay');?>">未付款</a>
			</li>
			<li role="presentation">
				<a href="<?php echo U('home/pay/isOut');?>">已出库</a>
			</li>
			<li role="presentation">
				<a href="<?php echo U('home/pay/isArrive');?>">已收货</a>
			</li>
			<li role="presentation" class="active">
				<a href="<?php echo U('home/pay/isRefund');?>">退款中</a>
			</li>
			<li role="presentation">
				<a href="<?php echo U('home/pay/isRefundDone');?>">完成退款</a>
			</li>
		</ul>
	</div>
	<div class="row">
		<table class="table">
			...
			<?php foreach($ofInfo as $item): ?>
			<tr>
				<td><?php echo ($item['usernames']); ?></td>
				<td><?php echo a4getDatetime($item['create_time'],'datetime');;?></td>
				<td>
					<a class="btn btn-link" href="<?php echo U('home/pay/detail');?>?no=<?php echo ($item['no']); ?>">
						<?php echo ($item['no']); ?>
					</a>
				</td>
				<td><?php echo ($item['price']); ?></td>
				<td>
					<?php if($item['is_need_invoice']==1): ?>
					需要
					<?php else: ?>
					不需要
					<?php endif; ?>
				</td>
				<td><?php echo ($item['order_form_status']); ?></td>
				<td><?php echo a4default($item['pay_type']);;?></td>
				<td>
					<a href="javascript:;" name="<?php echo ($item['no']); ?>" onclick="funRefundIsDone(this)">完成退款</a>
				</td>
			</tr>
			<?php endforeach; ?>
		</table>
		...
		<nav>
			<ul class="pagination">
				<li>
					<a href="<?php echo U('home/pay/index');?>?page_now=<?php echo ($_SESSION['bs']['limitPage']['pageNow']-1); ?>" aria-label="Previous">
						<span aria-hidden="true" class="glyphicon glyphicon-chevron-left"></span>
					</a>
				</li>
				<?php for($i=1;$i<=$_SESSION['bs']['limitPage']['pageMax'];$i++): ?>
				<?php if($i==$_SESSION['bs']['limitPage']['pageNow']): ?>
				<li><a href="<?php echo U('home/pay/index');?>?page_now=<?php echo ($i); ?>"><strong style="color: #D20000;"><?php echo ($i); ?></strong></a></li>
				<?php else: ?>
				<li><a href="<?php echo U('home/pay/index');?>?page_now=<?php echo ($i); ?>"><?php echo ($i); ?></a></li>
				<?php endif; ?>
				<?php endfor; ?>
				<li>
					<a href="<?php echo U('home/pay/index');?>?page_now=<?php echo ($_SESSION['bs']['limitPage']['pageNow']+1); ?>" aria-label="Next">
...
<script>
	 ...
	 * @param OBJ
	 */
	funRefundIsDone = function (OBJ) {
		$.ajax({
			url: '<?php echo U("home/pay/ajaxRefundIsDone");?>',
			type: 'post',
			data: 'no=' + OBJ.name,
			success: function (RET) {
				if (RET == 1) {
					showSuccessModal('批准退款', '批准退款成功', 2, function () {
						closeSuccessModal();
						location.reload();
					});
				} else {
					showFailModal('批准退款失败', RET);
				}
			}
		});
	}
