	...
	<div class="row">
		<div class="col-md-5">
			<div class="page-header">
				...
				<p>下单时间：<?php echo a4getDatetime($ofInfo['create_time']);;?></p>
				<p>金额：<?php echo ($ofInfo['price']); ?></p>
				<p>支付类型：<?php echo ($ofInfo['pay_type']); ?></p>
			</div>
		</div>
		<div class="col-offset-md-1 col-md-5">
			<div class="page-header">
				...
				<form id="formRefund">
					<input type="hidden" name="no" value="<?php echo ($ofInfo['no']); ?>">
...
<script language="javascript" type="text/javascript" charset="utf-8">
	document.getElementById('btnCreateRefund').onclick = function () {
		$.ajax({
			url: '<?php echo U("home/user/ajaxCreateRefund");?>',
			type: 'post',
			data: $('#formRefund').serialize(),
			success: function (RET) {
				if (RET == 1) {
					showSuccessModal('申请退款', '申请退款成功', 2, function () {
						location.herf = '<?php echo U("home/user/myOrderForm");?>';
					});
				} else {
