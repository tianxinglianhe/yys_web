...
<div class="container">
	...
	<table class="table table-responsive">
		...
		<form id="formOrderForm">
			...
			<input type="hidden" name="f_goods_id[]" value="<?php echo ($item['goods_id']); ?>">
			<tr>
				<td>
					<img src="<?php echo W_ROOT_IMG;?>1.jpg" alt="商品名称" height="100px">
					<span><?php echo ($item['name']); ?></span>
					<input type="hidden" name="f_norms_id[]" value="<?php echo ($item['norms_id']); ?>">
				</td>
				<td><?php echo ($item['norms_name']); ?></td>
				<td>￥ <?php echo ($item['price']); ?></td>
				<td>
					<h4>
				<span class="label label-danger">￥<?php echo ($item['number']*$item['price']); ?>
					<input type="hidden" name="price[]" value="<?php echo ($item['number']*$item['price']); ?>">
				</span>
					</h4>
				</td>
				<td>
					<input class="form-control" name="number[]" type="number" min="1" max="99" step="1" value="<?php echo ($item['number']); ?>">
				</td>
				<td>
					<a href="javascript:void(0);" name="<?php echo ($item['cart_id']); ?>" onclick="funDelCart(this)">删除</a>
					<br>
					<a href="javascript:void(0);" name="<?php echo ($item['goods_id']); ?>" onclick="funCart2Collection(this)">添加到我的收藏</a>
				</td>
			</tr>
			<?php endforeach; ?>
		</form>
...
<script language="javascript" type="text/javascript" charset="utf-8">
	...
	document.getElementById('btnCreateOrderForm').onclick = function () {
		$.ajax({
			url: '<?php echo U("home/user/ajaxCreateOrderForm");?>',
			type: 'post',
			data: $('#formOrderForm').serialize(),
			success: function (ret) {
				if (ret == 1) {
					location.href = '<?php echo U("home/pay/index");?>';
				} else {
	...
	funDelCart = function(OBJ){
		$.ajax({
			url: '<?php echo U("home/user/ajaxDelCart");?>',
			type: 'post',
			data: 'id=' + OBJ.name,
			success: function (ret) {
				if (ret == 1) {
					showSuccessModal('删除购物车商品', '删除成功', 3, function () {
						location.reload();
					});
				} else {
	...
	funCart2Collection = function(OBJ){
		$.ajax({
			url: '<?php echo U("home/user/ajaxCart2Collection");?>',
			type: 'post',
