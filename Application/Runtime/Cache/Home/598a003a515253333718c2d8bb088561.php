		...
		<div class="col-md-3">
			<div class="thumbnail">
				<a href="<?php echo U('home/goods/detail');?>?id=<?php echo ($item['id']); ?>">
					<img src="<?php echo W_ROOT_IMG;?>1.jpg" alt="<?php echo ($item['name']); ?>" height="200px">
				</a>
				<div class="caption">
					<h3>商品名称</h3>
					<span class="glyphicon glyphicon-yen" style="color: #D20000;">&nbsp;<?php echo ($item['price']); ?></span>

					<p>
						<a href="#" class="btn btn-default" id="btnDelCollection" name="<?php echo ($item['collection_id']); ?>">取消收藏</a>
					</p>
				</div>
			</div>
		</div>
...
<script language="javascript" type="text/javascript" charset="utf-8">
	document.getElementById('btnDelCollection').onclick = function () {
		$.ajax({
			url: '<?php echo U("home/user/ajaxDelCollection");?>',
			type: 'post',
			data: 'id=' + this.name,
			success: function (ret) {
				if (ret == 1) {
